...
template<typename T = std::string, typename B = char, typename S>
...
R base64_encode(const T* encoded_string, bool url = false) {
#if (defined(__cplusplus) && __cplusplus >= 201703L) || (defined(_HAS_CXX17) && _HAS_CXX17 == 1)
	std::basic_string_view<T> tmp{ encoded_string };
#else
	std::basic_string<T> tmp{ encoded_string };
#endif
...
template<typename R = std::string, typename T = wchar_t>
R base64_decode(const T* encoded_string, size_t in_len, bool remove_linebreaks = false) {
	...
	if(remove_linebreaks) {
		...
		return base64_decode<R, T>(copy.data(), copy.size(), false);
...
R base64_decode(const T* encoded_string, bool remove_linebreaks = false) {
	...
	std::basic_string<T> tmp{ encoded_string };
#endif
	return base64_decode<R, T>(tmp.data(), tmp.size(), remove_linebreaks);
}
#endif //BASE64_H